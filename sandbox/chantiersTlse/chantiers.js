...
function getLatestData () {
	const api = d3.json(
    'https://data.toulouse-metropole.fr/api/records/1.0/search/?dataset=chantiers-en-cours&rows=1000&facet=datedebut&facet=datefin&facet=declarant&facet=etat&facet=entreprise&facet=voie',
    data => {const chantiers = data.records.map(d => d.fields);
    	byKey =readCompress(readChantiers(chantiers))
// ------ set ToDay as the latest known sample
		ToDay = maxDates(byKey.map(function (e){
			return maxDates(e.date);}));
		ToDay.setHours(0);
...
function readChantiers(data){
	...
	data.forEach(function (e) {
		e.LatLng= {"lat" : e.geo_point_2d[0], "lng" : e.geo_point_2d[1]};
		//e.date = [new Date(e.datedebut), new Date(e.datefin)]
	...
	byKey.forEach(function (e) {
		...
		scoreGene(e);
		e.date = d3.extent([new Date(e.datedebut)]);
